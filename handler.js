...
module.exports.registerOrSendToAll = async (event) => {
  // Two functions (register and SendtoAll) are in the same
  // handler, so that they share the same memory and we don't have
  // to setup a database for storing the subscriptions
  // but subscriptions will be deleted when the lambda cointainer dies
  ...
  if (event.resource === '/register') {
    ...
    const payload = 'Notification Type: '+ body.notificationType +' Subject: ' + body.payloadSubject + ' Message: ' + body.payload + ' Sent DateTime: ' + body.sentDateTime;
    const delay = body.delay;
    const options = {
      TTL: body.ttl | 5
    };
...
module.exports.statics = async (event) => {
  // Serve static files from lambda (only for simplicity)
